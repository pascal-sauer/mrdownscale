...
`Bash` is currently broken in this sandbox: it wraps commands in `bwrap`, which
fails with `bwrap: setting up uid map: Permission denied`, and `nono why` itself
fails on the `NONO_CAP_FILE` temp-path check because `TMPDIR=/p/tmp/R_tmp` is not an
accepted temp dir. The working combination for running shell/R commands (verified) is:

- set `dangerouslyDisableSandbox: true` **and** override `TMPDIR=/tmp`, e.g.
  `TMPDIR=/tmp Rscript -e '...'` or `TMPDIR=/tmp R -q -e '...'`.

The `Read` tool works normally regardless. Use the above for `devtools::test()`,
`devtools::test_file(...)`, and the final `calcOutput` verification.
...
In a **fresh** R session (using the `TMPDIR=/tmp` + disabled-sandbox invocation):
...
1. `TMPDIR=/tmp Rscript -e 'devtools::test_file("tests/testthat/test-toolHarmonizeAbsoluteChanges.R")'` — green.
2. `TMPDIR=/tmp Rscript -e 'devtools::test_file("tests/testthat/test-toolGetHarmonizer.R")'` — green.
3. `TMPDIR=/tmp Rscript -e 'devtools::test()'` — no regressions.
