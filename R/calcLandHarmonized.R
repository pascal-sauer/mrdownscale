...
#' For harmonization = "absoluteChanges" this is a single year which has to
...
calcLandHarmonized <- function(input, target, harmonizationPeriod, harmonization) {
  ...
  if (harmonization == "absoluteChanges") {
    # this method uses a single harmonization year instead of a period and
    # needs no extrapolated target, so LandTargetExtrapolated is avoided here
  ...
  xInput <- toolEqualizeArea(xInput, xTarget[, hp1, ])
  ...
  toolExpectLessDiff(out[, getYears(out, as.integer = TRUE) <= hp1, ],
                     xTarget[, getYears(xTarget, as.integer = TRUE) <= hp1, ],
                     10^-5, "Returning reference data before harmonization period")

  # these checks assume out equals input after harmonizationPeriod[2], which
  # does not hold for absoluteChanges (single harmonization year, permanent offset)
  if (harmonization != "absoluteChanges") {
